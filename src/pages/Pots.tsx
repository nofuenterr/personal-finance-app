			...
			<main className="flex-1 overflow-hidden">
				<ScrollArea>
					<ul className="grid grid-cols-1 gap-6 md:grid-cols-[repeat(auto-fill,minmax(32.375rem,1fr))]">
						{pots.map((pot: Pot) => {
							return <PotCard key={pot.name} pot={pot} />;
						})}
					</ul>
				</ScrollArea>
			</main>
...
function PotCard({ pot }: { pot: Pot }) {
	...
	return (
		<li className="rounded-xl bg-white px-5 pt-6 pb-9.5 drop-shadow-[0_8px_24px_rgba(0,0,0,0.0.5)]">
			<div className={`text-${pot.theme} grid gap-8`}>
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-4">
						<div
							className={`size-4 rounded-full`}
							style={{ backgroundColor: `var(--color-${pot.theme})` }}
						></div>
						...
						>
							<Progress.Indicator
								...
								style={{
									...
									backgroundColor: `var(--color-${pot.theme})`,
								}}
							/>
						</Progress.Root>
						<div className="flex items-center justify-between">
